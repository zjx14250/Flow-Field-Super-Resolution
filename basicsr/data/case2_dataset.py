...
from basicsr.utils.constants import CASE2_MEAN, CASE2_STD
from basicsr.utils.registry import DATASET_REGISTRY


@DATASET_REGISTRY.register()
class Case2Dataset(Dataset):
    """Dataset for case2 HDF5 data.

    Args:
        opt (dict): Config for dataset. It contains:
            dataroot_lq (str): Path to LQ (low quality) h5 file (case2_cu_grid.h5)
            dataroot_gt (str): Path to GT (ground truth) h5 file (case2_chao_grid.h5)
            phase (str): 'train' or 'val'
    """

    def __init__(self, opt):
        super(Case2Dataset, self).__init__()
        self.opt = opt
        ...
        # Case2 global normalization statistics (computed from all resolutions)
        # Ensures consistent scale across cu/zhong/xi/chao datasets
        self.lq_mean = CASE2_MEAN
        self.lq_std = CASE2_STD
        self.gt_mean = CASE2_MEAN
        self.gt_std = CASE2_STD
